...
if [ "${action}" = "list-devices" ]; then
    ...
    if test -z "${denyRestart}"; then
        ...
        ssh -o 'BatchMode yes' -o 'StrictHostKeyChecking no' $nodeHost "test -r '/tmp/SB_RESTART_REQUIRED' && test -n \"\$(cat /tmp/SB_RESTART_REQUIRED)\" && echo 'info: system restart required, restarting now' && reboot || echo 'no system restart is necessary'"

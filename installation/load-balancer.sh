...
if [ "${action}" = "install" ]; then
    ...
    rsync -azve "ssh -o 'BatchMode yes' -o 'StrictHostKeyChecking no'" libfns.sh $lbHost:/tmp/
    ssh -o 'BatchMode yes' -o 'StrictHostKeyChecking no' $lbHost "source /tmp/libfns.sh && prepareLoadBalancer"
